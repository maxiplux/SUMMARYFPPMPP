package PracticeForStandard2017July.prob1;
...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		...
		return Stream.concat(list1.stream().filter( x ->  !list2.contains(x)),
				list2.stream().filter( x ->  !list1.contains(x))).collect(Collectors.toList());
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		//implement
		return list.stream().filter(x->x.getCity().equals("Fairfield")).map(x->x.getName()).collect(Collectors.toList());
	}

...
public class ProductDetailsWindow extends Stage {
	...
	public ProductDetailsWindow(ProductListWindow list, Product selectedProduct) {
		...
		addToCartButton.setOnAction(evt -> {
			...
			cart.setPrice((new Double(unitPrice)).toString());
			cart.setQuantity((new Integer(quant)).toString());
			//cart.setTotalPrice((new Double(totalPrice)).toString());
			ObservableList<ShoppingCart> cartData =
